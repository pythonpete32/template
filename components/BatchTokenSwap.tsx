...
import { X, Settings, ChevronDown, ArrowDown, AlertCircle } from "lucide-react";
...
export default function BatchTokenSwap() {
  ...
  const calculatePriceImpact = (amount: string): string => {
    if (
      !amount ||
    ...
    // Mock calculation - in a real app would be based on pool liquidity
    const amountValue = Number.parseFloat(amount);
    let impact = 0;

    if (amountValue > Number.parseFloat(fromToken.balance) * 0.3) {
      impact =
        0.5 +
        Math.min((amountValue / Number.parseFloat(fromToken.balance)) * 2, 10);
    } else {
      impact = Math.min(
        (amountValue / Number.parseFloat(fromToken.balance)) * 2,
        0.5
      );
    }
  ...
  const addToQueue = () => {
    ...
    const newSwap: SwapConfig = {
      ...
      priceImpact: calculatePriceImpact(fromAmount),
    };
      ...
      <Card className="mb-6">
        <CardContent className="pt-6">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                <p className="text-sm text-muted-foreground">To (estimated)</p>
              </div>
            ...
            {fromAmount && Number.parseFloat(fromAmount) > 0 && (
              <div className="space-y-2 pt-2 border-t text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{calculatePriceImpact(fromAmount)}</span>
                </div>
                <div className="flex justify-between">
            ...
            {showSettings && (
              <div className="p-4 border rounded-md space-y-4">
                ...
                <div className="flex gap-2 mb-2">
                  <Button
                    variant={slippageTolerance === 0.1 ? "default" : "neutral"}
                    size="sm"
                  ...
                  <Button
                    variant={slippageTolerance === 0.5 ? "default" : "neutral"}
                    size="sm"
                  ...
                  <Button
                    variant={slippageTolerance === 1.0 ? "default" : "neutral"}
                    size="sm"
              ...
              {swapQueue.map((swap) => (
                <Card key={swap.id} className="p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{swap.fromAmount}</span>
                  ...
                  <div className="text-xs text-muted-foreground mt-2">
                    Rate: {swap.exchangeRate} • Impact: {swap.priceImpact}
                  </div>
                </Card>
              ))}
            </div>
